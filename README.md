...
## Data directory
By default, scripts read data from:

`./data/<dataset>/`

Expected structure:

`./data/<dataset>/graph/graph_A.csv`  
`./data/<dataset>/graph/road_embed.txt`  
`./data/<dataset>/extra_file/*.json`  
`./data/<dataset>/road_network/*`  
`./data/<dataset>/gen_debug/*.bin` (for training with debug split)

You can also set a custom root path:
