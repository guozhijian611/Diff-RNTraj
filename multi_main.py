...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', type=str, default=None,
                        help='Root directory of processed data. Expected structure: '
                             '<data_root>/<dataset>/{graph,extra_file,road_network,gen_debug|gen_all}')
    ...
    rn_dir = os.path.join(path_dir, "road_network")
    UTG_file = os.path.join(path_dir, 'graph', 'graph_A.csv')
    pre_trained_road = os.path.join(path_dir, 'graph', 'road_embed.txt')
    if test_flag:
        train_trajs_dir = os.path.join(path_dir, 'gen_debug')
    else:
        train_trajs_dir = os.path.join(path_dir, 'gen_all')

    if test_flag:
        debug_eid = os.path.join(train_trajs_dir, 'eid_seqs.bin')
        debug_rate = os.path.join(train_trajs_dir, 'rate_seqs.bin')
        if not (os.path.exists(debug_eid) and os.path.exists(debug_rate)):
            fallback_dir = os.path.join(path_dir, 'gen_all')
            fallback_eid = os.path.join(fallback_dir, 'eid_seqs.bin')
            fallback_rate = os.path.join(fallback_dir, 'rate_seqs.bin')
            if os.path.exists(fallback_eid) and os.path.exists(fallback_rate):
                train_trajs_dir = fallback_dir
                print(f"gen_debug not found, fallback to: {train_trajs_dir}")

    required_paths = [
        UTG_file,
        pre_trained_road,
        os.path.join(extra_info_dir, 'raw_rn_dict.json'),
        os.path.join(extra_info_dir, 'new2raw_rid.json'),
        os.path.join(extra_info_dir, 'raw2new_rid.json'),
        os.path.join(extra_info_dir, 'rn_dict.json'),
        os.path.join(train_trajs_dir, 'eid_seqs.bin'),
        os.path.join(train_trajs_dir, 'rate_seqs.bin'),
    ]
    missing_paths = [p for p in required_paths if not os.path.exists(p)]
    if missing_paths:
        missing_msg = "\n".join(missing_paths)
        raise FileNotFoundError(
            f"Dataset files not found for '{opts.dataset}'.\n"
            f"Expected dataset root: {path_dir}\n"
            f"Missing required files:\n{missing_msg}\n\n"
            f"You can set --data_root to your processed data directory."
        )
    ...
    # load dataset
    with open(os.path.join(train_trajs_dir, 'eid_seqs.bin'), 'rb') as f:  #路段序列
        all_src_eid_seqs = pickle.load(f)
        f.close()
    # with open()
    with open(os.path.join(train_trajs_dir, 'rate_seqs.bin'), 'rb') as f:  #路段序列
